...
| 188   | 15 (alias advanced + multifile) | Fixed 1 bug: container alias (e.g. `type Values = [Int]`) with show/function calls caused false "type mismatch: expected List[Int], found Values" in check_fn_call_mismatches. Added deep_expand_aliases in check_fn_call_mismatches and check_indirect_call_mismatches. 13/15 pass (1 name collision, 1 missing stdlib method). | pending | ~11,515 |
