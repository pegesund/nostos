...
| 170     | 1 bug + ~30 (10988-11017) | **Bug**: Let-polymorphism too conservative for non-trivial lambda bodies: `wrap = (x) => [x]` couldn't be used at multiple types because internal Equal constraints between lambda-local vars blocked generalization. **Fix**: Distinguish "externally anchored" constraints (concrete types/outer-scope vars) from "internal-only" constraints (only lambda-local vars). Added `type_has_external_anchor()` check + transitive propagation. **Commit**: `e976faa`. **Test**: `tests/functions/let_polymorphism_wrap.nos`. **Probes (~30):** **MF complex (10):** let-poly cross-module, trait on built-in List cross-module, combined generic+trait+3-module, let-poly with closures, mutual recursion, transitive trait bounds, record+trait, variant creation+match, pipeline filter/map/sort, multi-param type. **SF stress (20):** let-poly in lambda, let-poly wrap+trait, composed let-poly, let-poly in foldl, let-poly captured in closure, trait on List using let-poly, recursive+let-poly, match arms let-poly, while+let-poly, try/catch+let-poly, trait on Map+lookup, trait on List+sort, two traits same built-in, trait on Option arithmetic, generic fn+List trait, pipeline+trait, nested trait calls, mutable+trait, record+List trait, combined let-poly+trait+pipeline. All 1544 tests pass. | Yes (e976faa) | ~11017 |
