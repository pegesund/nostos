...
| 186   | 20 (alias + multifile + inference) | Fixed 1 bug: type aliases in check_pending_method_calls pre-checks for map/filter/sum/product caused false Num trait error (Named{Id} classified as non-numeric instead of expanding alias to Int). 16/20 pass (3 syntax issues in probes, 1 fixed bug). Multi-file probes: alias chain, diamond, sort, variant, generic reuse all pass. | pending | ~11,484 |
