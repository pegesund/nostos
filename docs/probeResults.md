...
| 112     | 1 bug + ~50 (8527-8576) | **BUG FOUND AND FIXED:** Generic function with both `.map()` on a polymorphic parameter AND a recursive local function definition fails with "cannot resolve trait method `map` without type information". **Root cause:** After UFCS successfully compiles `.map()` via `compile_call`, a post-hoc ambiguity check (lines 14250-14287 in compile.rs) intended to distinguish List.map/Option.map/Result.map inspects the HM `inferred_expr_types` for the receiver. For monomorphized variants (name contains `$`), even though `param_types` has the correct concrete type (e.g., `List[(String, Int)]`), the stale HM type from the generic version is `Var(_)` (unresolved). The check treats this as ambiguous and throws `UnresolvedTraitMethod`, discarding the correct compilation. **Fix:** Before checking HM `inferred_expr_types`, verify whether the receiver variable has a concrete type in `param_types` (set during monomorphization). If so, trust that type and skip the stale HM check. Test: `tests/type_inference/generic_recursive_local_with_map.nos`. **Probes (50):** generic HOF, nested map+filter, Option.map chain, foldl with different accumulator, zip, string concat, chained boolean ops, mutual recursion, Result.map, filter+map pipeline, closure capture, zip self with map, recursive string building, sum via foldl, flatten via flatMap, toUpper, filter+length, safeHead, nested lambda, multi-file: generic Box, shapes records, Pair mapFst/mapSnd, diamond dependency, Student records, generic wrap at 3 types, 4-module chain, recursive Nat, lambda returning variant, Stack push/toList, function composition, Counter record, identity/squares/recSum. All 1529 tests pass. | `compile.rs`: trust param_types over stale HM in map/flatMap ambiguity check | ~8576 |
