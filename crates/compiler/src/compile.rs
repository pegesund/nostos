...
impl Compiler {
    ...
    /// Extract the field access chain from a nested FieldAccess expression.
    /// Returns (root_expr, field_chain) where field_chain is a list of field names
    /// from outermost to innermost.
    /// E.g., for `obj.inner.x`, returns (Var("obj"), ["inner", "x"])
    fn extract_field_chain<'a>(&self, expr: &'a Expr) -> (&'a Expr, Vec<&'a str>) {
        let mut fields = Vec::new();
        let mut current = expr;

        while let Expr::FieldAccess(obj, field, _) = current {
            fields.push(field.node.as_str());
            current = obj.as_ref();
        }
    ...
    /// Build the setter function name for a field.
    /// E.g., for type "Point" and field "x", returns "pointSetX"
    fn build_field_setter_name(&self, type_name: &str, field_name: &str) -> String {
        // Handle module-qualified type names
        let (module_prefix, base_type) = if let Some(dot_pos) = type_name.rfind('.') {
            (Some(&type_name[..dot_pos]), &type_name[dot_pos + 1..])
        } else {
            (None, type_name)
        };

        // Build type prefix with lowercase first letter
        let type_lower = base_type.chars().next()
            .map(|c| c.to_lowercase().collect::<String>() + &base_type[c.len_utf8()..])
            .unwrap_or_else(|| base_type.to_lowercase());

        // Build field suffix with uppercase first letter
        let field_cap = field_name.chars().next()
            .map(|c| c.to_uppercase().collect::<String>() + &field_name[c.len_utf8()..])
            .unwrap_or_else(|| field_name.to_uppercase());

        if let Some(module) = module_prefix {
            format!("{}.{}Set{}", module, type_lower, field_cap)
        } else {
            format!("{}Set{}", type_lower, field_cap)
        }
    }

    /// Check if a type is a mutable record
    fn is_mutable_record(&self, type_name: &str) -> bool {
        if let Some(type_info) = self.types.get(type_name) {
            if let TypeInfoKind::Record { mutable, .. } = &type_info.kind {
                return *mutable;
            }
        }
        false
    ...
    fn compile_assign(&mut self, target: &AssignTarget, value: &Expr) -> Result<Reg, CompileError> {
        ...
        match target {
            ...
            AssignTarget::Field(obj, field) => {
                // Check for custom field setter dispatch: obj.field = v -> typeSetField(obj, v)
                // Also handles nested field access: obj.inner.x = v

                // Collect the field chain from root variable to innermost field
                // For obj.inner.x = v, we have obj=FieldAccess(Var("obj"), "inner"), field="x"
                // Chain should be: [("inner", inner_type), ("x", x_type)]
                let mut field_chain: Vec<(String, String)> = vec![(field.node.clone(), String::new())];
                let mut current = obj.as_ref();

                while let Expr::FieldAccess(parent, parent_field, _) = current {
                    field_chain.push((parent_field.node.clone(), String::new()));
                    current = parent.as_ref();
                }

                // Reverse to get root-to-innermost order
                field_chain.reverse();

                // Check if root is a variable (required for updating)
                if let Expr::Var(root_ident) = current {
                    // Get type of root variable
                    if let Some(root_type) = self.expr_type_name(current) {
                        // Fill in types for each level of the chain
                        let mut level_types: Vec<String> = vec![root_type.clone()];
                        let mut current_type = root_type.clone();

                        for (field_name, _) in &field_chain {
                            // Get the type of this field from the current type's record definition
                            if let Some(type_info) = self.types.get(&current_type) {
                                if let TypeInfoKind::Record { fields, .. } = &type_info.kind {
                                    if let Some((_, ftype)) = fields.iter().find(|(n, _)| n == field_name) {
                                        level_types.push(ftype.clone());
                                        current_type = ftype.clone();
                                    } else {
                                        break; // Field not found, will fall through to default
                                    }
                                } else {
                                    break; // Not a record, will fall through to default
                                }
                            } else {
                                break; // Type not found, will fall through to default
                            }
                        }

                        // Check if all types are mutable records (except the innermost value type)
                        let all_mutable = level_types.iter().take(level_types.len().saturating_sub(1))
                            .all(|t| self.is_mutable_record(t));

                        if all_mutable && level_types.len() == field_chain.len() + 1 {
                            // Check if we have setters for all levels
                            let mut all_setters_exist = true;
                            let mut setter_funcs: Vec<String> = Vec::new();

                            for (i, (field_name, _)) in field_chain.iter().enumerate() {
                                let obj_type = &level_types[i];
                                let setter_name = self.build_field_setter_name(obj_type, field_name);
                                let func_arg_types = vec![Some(obj_type.clone()), None];

                                if let Some(resolved_func) = self.resolve_function_call(&setter_name, &func_arg_types) {
                                    if self.functions.contains_key(&resolved_func) {
                                        setter_funcs.push(resolved_func);
                                    } else {
                                        all_setters_exist = false;
                                        break;
                                    }
                                } else {
                                    all_setters_exist = false;
                            ...
                            if all_setters_exist && !setter_funcs.is_empty() {
                                // Compile root variable and intermediate field accesses
                                let root_reg = self.compile_expr_tail(current, false)?;
                                let mut object_regs: Vec<Reg> = vec![root_reg];

                                // Compile intermediate field accesses (all but the last)
                                for (field_name, _) in field_chain.iter().take(field_chain.len() - 1) {
                                    // Create a FieldAccess expression on the fly to compile
                                    let field_idx = self.chunk.add_constant(Value::String(Arc::new(field_name.clone())));
                                    let prev_reg = *object_regs.last().unwrap();
                                    let field_reg = self.alloc_reg();
                                    self.chunk.emit(Instruction::GetField(field_reg, prev_reg, field_idx), 0);
                                    object_regs.push(field_reg);
                                }

                                // Now chain setter calls from innermost to outermost
                                let mut current_value_reg = value_reg;

                                for i in (0..setter_funcs.len()).rev() {
                                    let setter_func = &setter_funcs[i];
                                    let obj_reg = object_regs[i];
                                    let result_reg = self.alloc_reg();

                                    let func_idx = *self.function_indices.get(setter_func)
                                        .expect("Function should have been assigned an index");

                                    self.current_fn_calls.insert(setter_func.clone());
                                    self.chunk.emit(Instruction::CallDirect(result_reg, func_idx, vec![obj_reg, current_value_reg].into()), 0);

                                    current_value_reg = result_reg;
                                }

                                // Update the root variable with the final result
                                if let Some(local_info) = self.locals.get(&root_ident.node) {
                                    let local_reg = local_info.reg;
                                    self.chunk.emit(Instruction::Move(local_reg, current_value_reg), 0);
                                }
                ...
                // Default: use built-in SetField instruction
                let obj_reg = self.compile_expr_tail(obj, false)?;
                let field_idx = self.chunk.add_constant(Value::String(Arc::new(field.node.clone())));
