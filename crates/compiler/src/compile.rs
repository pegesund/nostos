...
impl Compiler {
    ...
    fn compile_expr_tail(&mut self, expr: &Expr, is_tail: bool) -> Result<Reg, CompileError> {
        ...
        match expr {
            ...
            Expr::Record(type_name, fields, span) => {
                ...
                if fields.len() >= 2 {
                    if let RecordField::Positional(base_expr) = &fields[0] {
                        ...
                        if has_named {
                            // This is a record update - extract named fields only
                            let named_fields: Vec<RecordField> = fields[1..].iter()
                                .filter(|f| matches!(f, RecordField::Named(_, _)))
                                .cloned()
                                .collect();
                            return self.compile_record_update(&qualified_type, base_expr, &named_fields);
                        }
                    }
                }
            ...
            // Record update (explicit AST node, currently not generated by parser)
