...
fn ident() -> impl Parser<Token, Ident, Error = Simple<Token>> + Clone {
    filter_map(|span, tok| match tok {
        ...
        // 'from' is used in extern declarations (extern fn foo() -> T from "lib") but is not truly
        // reserved as an identifier - allow it as a variable/pattern name
        Token::From => Ok(make_ident("from".to_string(), to_span(span))),
        ...
        Token::Extern | Token::Quote | Token::Template =>
...
pub fn expr() -> impl Parser<Token, Expr, Error = Simple<Token>> + Clone {
    recursive(|expr| {
        ...
        // Type annotations are stored in the pattern as TypeAnnotated for type checking.
        let lambda_param = pattern()
            .then(just(Token::Colon).ignore_then(type_expr()).or_not())
            .map_with_span(|(pat, type_annotation), span| {
                match type_annotation {
                    Some(ty) => {
                        let s = to_span(span);
                        Pattern::TypeAnnotated(Box::new(pat), ty, s)
                    }
                    None => pat,
                }
            });
