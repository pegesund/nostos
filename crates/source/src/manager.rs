...
use crate::module::{Module, ModulePath, module_path_to_string};
...
impl SourceManager {
    ...
    fn load_from_defs_dir(&mut self, defs_dir: &Path) -> Result<(), String> {
            ...
            .filter(|e| e.path().extension().map(|ext| ext == "nos").unwrap_or(false))
        {
            ...
            // Skip _imports.nos for now, handle separately
            if file_name == "_imports" {
                continue;
            }
    ...
    pub fn get_source(&self, name: &str) -> Option<String> {
        ...
        if let Some(module_key) = self.def_index.get(name) {
            if let Some(module) = self.modules.get(module_key) {
                if let Some(group) = module.get_definition(name) {
        ...
        if let Some(group) = self.repl_defs.get(name) {
            return Some(group.combined_source());
        }
