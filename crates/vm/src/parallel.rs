...
impl ThreadWorker {
    ...
    fn execute_instruction(
        ...
        match instr {
            ...
            Construct(dst, type_reg, json_reg) => {
                // Construct a typed value from type name and Json
                let (type_name, json_val) = {
                    ...
                    let jv = reg!(*json_reg).clone();
                    (tn, jv)
                };
                match self.construct_from_json(local_id, &type_name, json_val) {
                    Ok(result) => {
                        let proc = self.get_process_mut(local_id).unwrap();
                        set_reg!(*dst, result);
                    }
    ...
    fn construct_from_json(&mut self, local_id: u64, type_name: &str, json: GcValue) -> Result<GcValue, RuntimeError> {
        ...
        let pairs = self.extract_json_object_pairs(local_id, &json)?;
        ...
        if type_val.constructors.is_empty() {
            ...
            if ctor.fields.is_empty() {
                ...
                // Variant with fields
                let field_pairs = self.extract_json_object_pairs(local_id, fields_json)?;
                let mut field_values = Vec::new();
    ...
    fn extract_json_object_pairs(&self, local_id: u64, json: &GcValue) -> Result<Vec<(String, GcValue)>, RuntimeError> {
        match json {
            GcValue::Variant(var_ptr) => {
                let proc = self.get_process(local_id).unwrap();
                let variant = proc.heap.get_variant(*var_ptr)
                    .ok_or_else(|| RuntimeError::Panic("Invalid variant".to_string()))?;

                if variant.constructor.as_str() != "Object" {
                    return Err(RuntimeError::Panic(format!(
    ...
    fn json_to_primitive(&mut self, local_id: u64, json: &GcValue, expected_type: &str) -> Result<GcValue, RuntimeError> {
        match json {
            GcValue::Variant(var_ptr) => {
                let (ctor, fields) = {
                    let proc = self.get_process(local_id).unwrap();
                    let variant = proc.heap.get_variant(*var_ptr)
                        .ok_or_else(|| RuntimeError::Panic("Invalid variant".to_string()))?;
                    (variant.constructor.clone(), variant.fields.clone())
                };

                let ctor_str = ctor.as_str();
                if ctor_str == "Null" {
                    Ok(GcValue::Unit)
                } else if ctor_str == "Bool" {
                    if fields.is_empty() {
                ...
                } else if ctor_str == "String" {
                    if fields.is_empty() {
                        let proc = self.get_process_mut(local_id).unwrap();
                        Ok(GcValue::String(proc.heap.alloc_string(String::new())))
                    } else {
                        match &fields[0] {
                            ...
                            GcValue::List(list) => {
                                ...
                                if expected_type.starts_with('(') && expected_type.ends_with(')') {
                                    ...
                                    for (i, item) in list_items.iter().enumerate() {
                                        ...
                                        items.push(self.json_to_primitive(local_id, item, elem_type)?);
                                    }
                                    let proc = self.get_process_mut(local_id).unwrap();
                                    let tup_ptr = proc.heap.alloc_tuple(items);
                                    Ok(GcValue::Tuple(tup_ptr))
                                } else {
                                    // Recursively convert each item as list
                                    ...
                                    for item in list_items {
                                        items.push(self.json_to_primitive(local_id, &item, &inner_type)?);
                                    }
                ...
                } else if ctor_str == "Object" {
                    // Check if this is a nested record or variant
                    let _pairs = self.extract_json_object_pairs(local_id, json)?;

                    // Try to look up as a type
                    let type_info = self.shared.types.get(expected_type).cloned()
                        .or_else(|| self.shared.dynamic_types.read().get(expected_type).cloned())
                        .or_else(|| self.shared.stdlib_types.read().get(expected_type).cloned());

                    if type_info.is_some() {
                        // Recursively construct the nested type
                        return self.construct_from_json(local_id, expected_type, json.clone());
                    }
