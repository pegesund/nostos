...
impl Runtime {
    ...
    fn execute_local_instruction_inline(
        ...
        match instr {
            ...
            | Instruction::FileAppend(_, _, _)
            | Instruction::FileOpen(_, _, _)
            | Instruction::FileWrite(_, _, _)
            | Instruction::FileRead(_, _, _)
            | Instruction::FileReadLine(_, _)
            | Instruction::FileFlush(_, _)
            | Instruction::FileClose(_, _)
            | Instruction::FileSeek(_, _, _, _)
            | Instruction::DirCreate(_, _)
            | Instruction::DirCreateAll(_, _)
            | Instruction::DirList(_, _)
            | Instruction::DirRemove(_, _)
            | Instruction::DirRemoveAll(_, _)
            | Instruction::FileExists(_, _)
            | Instruction::DirExists(_, _)
            | Instruction::FileRemove(_, _)
            | Instruction::FileRename(_, _, _)
            | Instruction::FileCopy(_, _, _)
            | Instruction::FileSize(_, _)
            ...
            | Instruction::ServerBind(_, _)
            | Instruction::ServerAccept(_, _)
            | Instruction::ServerRespond(_, _, _, _, _)
            | Instruction::ServerClose(_, _)
