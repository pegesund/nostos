...
pub fn run_tui(args: &[String]) -> ExitCode {
    ...
    match engine.discover_nostlets() {
        Ok(count) if count > 0 => {
            // Resolve nostlet metadata (name, description) now that engine is ready
            engine.resolve_nostlet_metadata();
        }
        Err(e) => {
            eprintln!("Warning: Failed to discover nostlets: {}", e);
        }
        _ => {}
    }
    ...
    // Global Ctrl+N to open nostlet picker
...
/// Show the nostlet picker dialog
fn show_nostlet_picker(s: &mut Cursive, engine: Rc<RefCell<ReplEngine>>) {
    ...
    let dialog = Dialog::around(
        select
            .with_name("nostlet_list")
            .scrollable()
            .fixed_size((60, 15))
    )
    .title("Select Nostlet (Ctrl+N)")
...
fn create_nostlet_view(engine: &Rc<RefCell<ReplEngine>>, module_name: &str) -> impl View {
    ...
    let (render_fn, key_handler_fn, title) = {
        ...
        if let Some(info) = eng.get_nostlet(module_name) {
            (info.render_fn.clone(), info.key_handler_fn.clone(), info.name.clone())
        } else {
            // Fallback if nostlet not found
            (format!("{}.render", module_name), format!("{}.onKey", module_name), module_name.to_string())
        }
    };
    ...
    let editor_with_events = OnEventView::new(editor.with_name(&editor_id))
        ...
        .on_event(Event::CtrlChar('o'), move |s| {
            ...
            if engine.is_metadata(&name_for_compile) {
                debug_log(&format!("Ctrl+O: Saving metadata: {}", name_for_compile));
                match engine.save_metadata(&name_for_compile, &content) {
                    Ok(()) => {
                        drop(engine);
                        log_to_repl(s, &format!("Saved {}", name_for_compile));
                        s.call_on_name(&editor_id_compile, |v: &mut CodeEditor| v.mark_saved());
                    }
                    Err(e) => {
                        drop(engine);
                        log_to_repl(s, &format!("Error saving {}: {}", name_for_compile, e));
                    }
                }
                return;
            }
            ...
            if engine.has_source_manager() {
                ...
                let compile_result = match compile_result {
                    ...
                    Err(panic_info) => {
                        ...
                        debug_log(&format!("Ctrl+O eval_in_module PANIC: {}", panic_msg));
                        Err(format!("Internal error (panic): {}", panic_msg))
                    }
                };
                ...
                match compile_result {
                    Ok(_output) => {
                        ...
                        match save_result {
                            Ok(_saved_names) => {
                                // Mark as compiled
                                let module_prefix = if let Some(dot_pos) = name_for_compile.rfind('.') {
                                    format!("{}.", &name_for_compile[..dot_pos])
                                } else {
                                    String::new()
                                };
                            ...
                            Err(save_error) => {
                                drop(engine);
                                log_to_repl(s, &format!("Save error: {}", save_error));
                            }
                        }
                    }
                    Err(compile_error) => {
                        ...
                        // Set error in editor for visual feedback
                        s.call_on_name(&editor_id_compile, |v: &mut CodeEditor| {
    ...
    for item in &items {
        let label: StyledString = match item {
            ...
            BrowserItem::Function { name, signature, doc, eval_created, is_public } => {
                ...
                // Add status prefix with colored symbol (back, front order in ColorStyle::new)
                match &compile_status {
                    Some(CompileStatus::CompileError(_)) => {
                        styled.append_styled("✗ ", Style::from(ColorStyle::new(Color::TerminalDefault, Color::Light(BaseColor::Red))));
                    }
                    Some(CompileStatus::Stale { .. }) => {
                        styled.append_styled("○ ", Style::from(ColorStyle::new(Color::TerminalDefault, Color::Light(BaseColor::Yellow))));
                    }
                    Some(CompileStatus::Compiled) => {
                        styled.append_styled("✓ ", Style::from(ColorStyle::new(Color::TerminalDefault, Color::Light(BaseColor::Green))));
                    }
                    _ => {
